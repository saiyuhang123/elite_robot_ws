...
class CameraDetector(Node):
    ...
    def __init__(self):
        ...
        self.latest_depth = None     # 16UC1，Percipio 原始值 0.25mm/LSB，已与彩色图对齐
        self.latest_p_cam = None
        # Percipio 驱动只在 image_raw 有订阅者时才发布 camera_info，三个订阅缺一不可
    ...
    def position_from_depth(self, u, v, fw, fh):
        ...
        if valid.size < 20:
            return None
        z = float(np.median(valid)) * 0.25 / 1000.0  # Percipio: 0.25mm/LSB → 米
        if z < 0.15 or z > 3.0:
            return None
